...
const SORTS = [
  { id: 'NEWEST', label: 'Newest first', compare: (a, b) => Date.parse(b.createdAt) - Date.parse(a.createdAt) },
  { id: 'OLDEST', label: 'Oldest first', compare: (a, b) => Date.parse(a.createdAt) - Date.parse(b.createdAt) },
  { id: 'VALUE', label: 'Highest fare', compare: (a, b) => (b.pricing?.total || 0) - (a.pricing?.total || 0) },
  { id: 'DISTANCE', label: 'Longest route', compare: (a, b) => (b.route?.distanceKm || 0) - (a.route?.distanceKm || 0) }
];
...
/**
 * §18/§26 — the dispatch console: the board of every delivery in the system and the
 * levers that move the selected one. Every mutation here goes through the backend
...
export default function AdminDeliveries() {
  ...
  const mayDispatch = can(user, PERMISSION.DISPATCH);

  const visible = useMemo(() => {
    const needle = query.trim().toLowerCase();
    const compare = (SORTS.find((option) => option.id === sort) || SORTS[0]).compare;
    return orders
      .filter((order) => {
  ...
  return (
    <>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px', marginBottom: 'clamp(18px,2.4vw,26px)' }}>
        {tiles.map((tile) => (
          <StatTile
            key={tile.label}
            label={tile.label}
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '12px', alignItems: 'center', marginBottom: '14px' }}>
            <div style={{ position: 'relative', flex: '1 1 240px' }}>
              <input
                ...
                placeholder="Search parcel, customer, courier or place"
            ...
            <label style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <span style={{ ...control.label, marginBottom: 0 }}>Sort</span>
              <select
                value={sort}
                onChange={(event) => setSort(event.target.value)}
                aria-label="Sort deliveries"
                style={{ ...control.field, width: 'auto', height: '48px', paddingRight: '12px', cursor: 'pointer' }}
              >
                {SORTS.map((option) => (
