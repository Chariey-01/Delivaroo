    ...
    >
      ...
      <Link to={`/orders/${order.id}`} style={{ display: 'flex', minWidth: 0, gap: '12px', color: color.ink }}>
        ...
        <span style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: '10px' }}>
          <span style={{ fontSize: '15.5px', fontWeight: 700, letterSpacing: 0 }}>{order.pickup.name}</span>
          <span style={{ fontSize: '15.5px', fontWeight: 700, letterSpacing: 0 }}>{order.destination.name}</span>
        </span>
      </Link>
